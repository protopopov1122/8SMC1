...
#include "calx/ctrl-lib/conf/Event.h"
#include <memory>

namespace CalX {

	class ConfiguationFlatDictionary
	    : public EventSource<std::shared_ptr<FlatDictionaryListener>> {
	 public:
	...
	class ConfigurationCatalogue
	    : public EventSource<std::shared_ptr<CatalogueListener>> {
	 public:
		virtual ~ConfigurationCatalogue() = default;
		virtual ConfiguationFlatDictionary *getEntry(const std::string &,
		                                             bool = true) = 0;
