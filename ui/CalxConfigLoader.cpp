...
namespace CalXUI {
	...
	void CalxConfigLoader::load() {
		...
		while (!fname.empty()) {
			std::string path = this->filesys.URLToFileName(fname).GetFullPath().ToStdString();
			...
			if (!cnf.good()) {
				...
				name = conf->getEntry("config")->getString("name", this->filesys.URLToFileName(fname).GetFullName().ToStdString());
