...
namespace CalXUI {
	...
	CalxCoordAdjustAction::CalxCoordAdjustAction(CalxCoordAdjustDialog *dialog, CoordHandle *handle,
		CalxAdjustDirection dir, double step, double speed) :
		dialog(dialog), handle(handle), direction(dir), step(step), speed(speed) {}
	...
	void CalxCoordAdjustAction::perform(SystemManager *sysman) {
		...
		handle->open_session();
		wxGetApp().getErrorHandler()->handle(handle->getFloatPlane()->relativeMove(dest, speed, false));
	...
	CalxCoordAdjustMoveAction::CalxCoordAdjustMoveAction(CalxCoordAdjustDialog *dialog, CoordHandle *handle,
	...
	void CalxCoordAdjustMoveAction::perform(SystemManager *sysman) {
		dialog->setEnabled(false);
		handle->open_session();
		wxGetApp().getErrorHandler()->handle(handle->getFloatPlane()->move(dest, speed, false));
		handle->close_session();
		dialog->setEnabled(true);
	}
	...
	CalxCoordAdjustRepaintTimer::CalxCoordAdjustRepaintTimer(CalxCoordAdjustDialog *dialog)
	...
	CalxCoordAdjustViewer::CalxCoordAdjustViewer(wxWindow *win, wxWindowID id,
		...
		wxWindow::wxWindow(win, id), handle(handle),
	...
	CalxCoordAdjustDialog::CalxCoordAdjustDialog(wxWindow *win, wxWindowID id, CoordHandle *handle) :
		wxDialog::wxDialog(win , id, __("Adjust dialog"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER),
		handle(handle) {
		...
		this->viewer = new CalxCoordAdjustViewer(this, wxID_ANY, handle, wxSize(500, 500));
        ...
        this->stepSpin = new wxSpinCtrlDouble(prmsPanel, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                      wxDefaultSize, wxSP_ARROW_KEYS, wxGetApp().getUnitPrecision(), INT_MAX,
									  wxGetApp().getSystemManager()->getConfiguration()->
                                        getEntry("ui")->getReal("adjust_step", 2.5),
									  wxGetApp().getUnitPrecision());
        this->speedSpin = new wxSpinCtrlDouble(prmsPanel, wxID_ANY, wxEmptyString, wxDefaultPosition,
                                     wxDefaultSize, wxSP_ARROW_KEYS, wxGetApp().getSpeedPrecision(),
                                     wxGetApp().getSystemManager()->getConfiguration()->
                                        getEntry("units")->getReal("unit_speed", 4000.0),
                                     wxGetApp().getSystemManager()->getConfiguration()->
                                        getEntry("units")->getReal("unit_speed", 4000.0), wxGetApp().getSpeedPrecision());
		prmsSizer->Add(new wxStaticText(prmsPanel, wxID_ANY, __("Position") + std::string(":")), 0, wxALIGN_RIGHT | wxRIGHT, 5);
		prmsSizer->Add(this->position);
		prmsSizer->Add(new wxStaticText(prmsPanel, wxID_ANY, ""));
		prmsSizer->Add(new wxStaticText(prmsPanel, wxID_ANY, __("Step") + std::string(":")), 0, wxALIGN_RIGHT | wxRIGHT, 5);
		prmsSizer->Add(this->stepSpin, 0, wxALL | wxEXPAND);
		prmsSizer->Add(new wxStaticText(prmsPanel, wxID_ANY, wxGetApp().getUnits()));
		prmsSizer->Add(new wxStaticText(prmsPanel, wxID_ANY, __("Speed") + std::string(":")), 0, wxALIGN_RIGHT | wxRIGHT, 5);
		prmsSizer->Add(this->speedSpin, 0, wxALL | wxEXPAND);	
		prmsSizer->Add(new wxStaticText(prmsPanel, wxID_ANY, wxGetApp().getSpeedUnits()));	
		...
		this->queue->Run();
		int_conf_t interval = wxGetApp().getSystemManager()->getConfiguration()->getEntry("ui")->getInt("watcher_render_interval", 50);
		if (interval != -1) {
		...
		this->viewer->Bind(wxEVT_LEFT_DOWN, &CalxCoordAdjustDialog::OnMouseClick, this);
	...
	void CalxCoordAdjustDialog::adjust(CalxAdjustDirection dir) {
		this->queue->addAction(new CalxCoordAdjustAction(this, this->handle, dir, this->stepSpin->GetValue(), this->speedSpin->GetValue()));
	}
	
	void CalxCoordAdjustDialog::setEnabled(bool e) {
		for (auto i = buttonPanel->GetChildren().begin(); i != buttonPanel->GetChildren().end(); ++i) {
	...
	void CalxCoordAdjustDialog::OnMouseClick(wxMouseEvent &evt) {
		...
		if (mouse.x >= 0 &&
			...
			this->queue->addAction(new CalxCoordAdjustMoveAction(this, this->handle, dest, this->speedSpin->GetValue()));
		}
	}
	...
	void CalxCoordAdjustDialog::updateUI() {
		...
		this->position->SetLabel(FORMAT(__("%sx%s %s"), wxGetApp().formatDouble(pos.x), wxGetApp().formatDouble(pos.y),
			wxGetApp().getUnits()));
	}
}