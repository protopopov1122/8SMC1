...
namespace CalXUI {
  ...
  CalxCoordMoveAction::CalxCoordMoveAction(CalxCoordCtrl *ctrl,
										   CoordHandle *handle, bool jump,
										   bool relative, motor_point_t dest,
										   float speed) {
	this->ctrl = ctrl;
	this->handle = handle;
	this->jump = jump;
  ...
  void CalxCoordMoveAction::perform(SystemManager *sysman) {
	...
	if (relative) {
	  wxGetApp().getErrorHandler()->handle(
		  handle->relativeMove(dest, speed, jump));
	} else {
  ...
  CalxCoordFloatMoveAction::CalxCoordFloatMoveAction(CalxCoordCtrl *ctrl,
													 CoordHandle *handle,
  ...
  CalxCoordArcAction::CalxCoordArcAction(CalxCoordCtrl *ctrl,
										 CoordHandle *handle, bool relative,
										 motor_point_t dest, motor_point_t cen,
										 int splitter, float speed,
										 bool clockwise) {
	this->ctrl = ctrl;
	this->handle = handle;
	this->relative = relative;
  ...
  void CalxCoordArcAction::perform(SystemManager *sysman) {
	...
	if (relative) {
	  wxGetApp().getErrorHandler()->handle(
		  handle->relativeArc(dest, cen, splitter, speed, clockwise));
	} else {
	  wxGetApp().getErrorHandler()->handle(
		  handle->arc(dest, cen, splitter, speed, clockwise));
	}
  ...
  CalxCoordFloatArcAction::CalxCoordFloatArcAction(
	  CalxCoordCtrl *ctrl, CoordHandle *handle, bool relative,
	  coord_point_t dest, coord_point_t cen, int splitter, double speed,
	  bool clockwise) {
	this->ctrl = ctrl;
	this->handle = handle;
	this->relative = relative;
  ...
  CalxCoordGraphAction::CalxCoordGraphAction(CalxCoordCtrl *ctrl,
											 CoordHandle *handle,
											 CoordTranslator *trans,
											 GraphBuilder *builder, float speed,
											 bool use_float) {
	this->ctrl = ctrl;
	this->handle = handle;
  ...
  CalxCoordCalibrateAction::CalxCoordCalibrateAction(CalxCoordCtrl *ctrl,
													 CoordHandle *handle,
													 TrailerId tr) {
	this->ctrl = ctrl;
	this->handle = handle;
  ...
  CalxCoordMeasureAction::CalxCoordMeasureAction(CalxCoordCtrl *ctrl,
												 CoordHandle *handle,
												 TrailerId tr) {
	this->ctrl = ctrl;
	this->handle = handle;
  ...
  void CalxCoordMeasureAction::perform(SystemManager *sysman) {
	...
	this->ctrl->setMaster(true);
	wxGetApp().getErrorHandler()->handle(
		handle->getController()->measure(trailer));
	this->ctrl->setMaster(false);
	handle->close_session();
  }
  ...
  CalxCoordConfigureAction::CalxCoordConfigureAction(
	  CalxCoordCtrl *ctrl, CoordHandle *handle, bool jump, bool relative,
	  coord_point_t dest, float speed, bool use_float) {
	this->ctrl = ctrl;
	this->handle = handle;
	this->jump = jump;
	this->relative = relative;
