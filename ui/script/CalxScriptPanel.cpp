...
namespace CalXUI {
	...
	class CalxScriptActionWrapper : public CalxAction {
	 public:
		CalxScriptActionWrapper(CalxScriptPanel *panel, std::unique_ptr<CalxAction> action)
	...
	CalxScriptPanel::CalxScriptPanel(wxWindow *win, wxWindowID id,
		...
		wxButton *loadButton = new wxButton(scriptListPanel, wxID_ANY, __("Load script"));
		scriptListSizer->Add(loadButton, 0, wxALL | wxEXPAND);
		loadButton->Bind(wxEVT_BUTTON, &CalxScriptPanel::OnLoadScriptClick, this);
		wxButton *shellButton = new wxButton(scriptListPanel, wxID_ANY, __("Open shell"));
		scriptListSizer->Add(shellButton, 0, wxALL | wxEXPAND);
		shellButton->Bind(wxEVT_BUTTON, &CalxScriptPanel::OnOpenShellClick, this);
		wxButton *removeButton = new wxButton(scriptListPanel, wxID_ANY, __("Remove"));
		scriptListSizer->Add(removeButton, 0, wxALL | wxEXPAND);
		removeButton->Bind(wxEVT_BUTTON, &CalxScriptPanel::OnRemoveScriptClick, this);
	...
	void CalxScriptPanel::sendAction(std::unique_ptr<CalxAction> action) {
		this->queue->addAction(std::make_unique<CalxScriptActionWrapper>(this, std::move(action)));
	}
	...
	void CalxScriptPanel::OnLoadScriptClick(wxCommandEvent &evt) {
		wxFileDialog *dialog =
			new wxFileDialog(this, __("Import script file"), "", "", "",
							 wxFD_OPEN);
		if (dialog->ShowModal() == wxID_OK) {
			...
			std::unique_ptr<CalXScript> script = this->scriptFactory.openFile(this->env, path);
			this->addHandle(path, new CalxFileScriptHandle(this->scriptPanel, wxID_ANY, this, code, std::move(script)));
		}
	}
	
	void CalxScriptPanel::OnOpenShellClick(wxCommandEvent &evt) {
		std::unique_ptr<CalXScript> script = this->scriptFactory.createShell(this->env);
		this->addHandle(FORMAT(__("Script shell #%d"), this->scripts.size()), new CalxScriptShellHandle(this->scriptPanel, wxID_ANY, this, std::move(script)));
	}
