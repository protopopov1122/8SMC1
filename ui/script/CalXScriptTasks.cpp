...
namespace CalX::UI {
	...
	class CalxGCodeStringTaskBuilder : public CalxTaskFactory {
	 public:
		CalxGCodeStringTaskBuilder(const std::string &title,
		                           const std::string &gcode)
		    : title(title), gcode(gcode) {
			this->translator = getDefaultTranslator();
		}
	...
	class CalxLinearTaskBuilder : public CalxTaskFactory {
	 public:
		CalxLinearTaskBuilder(std::size_t id, CalxLinearTaskParameters &prms)
		    : id(id), prms(prms) {
			this->translator = getDefaultTranslator();
		}
	...
	CalXAppScriptTasks::CalXAppScriptTasks(CalxApp &app) : app(app) {}
	...
	void CalXAppScriptTasks::newGCodeFile(const std::string &title,
	                                      const std::string &path) {
		this->app.getMainFrame()->getTaskList()->attachTask(
		    title, std::make_shared<CalxGCodeFileTaskBuilder>(path));
	}

	void CalXAppScriptTasks::newGCode(const std::string &title,
	                                  const std::string &path) {
		this->app.getMainFrame()->getTaskList()->attachTask(
		    title, std::make_shared<CalxGCodeStringTaskBuilder>(title, path));
	}

	void CalXAppScriptTasks::newLinear(const std::string &title,
	                                   CalxLinearTaskParameters &prms) {
		this->app.getMainFrame()->getTaskList()->attachTask(
