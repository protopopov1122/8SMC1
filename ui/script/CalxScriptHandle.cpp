...
namespace CalXUI {
	...
	CalxFileScriptHandle::CalxFileScriptHandle(wxWindow *win, wxWindowID id,
		CalxActionReceiver *recv, std::string code, std::unique_ptr<CalXScript> script)
		: CalxScriptHandle::CalxScriptHandle(win, id), recv(recv), script(std::move(script)) {
		...
		wxStaticText *hookLabel = new wxStaticText(this, wxID_ANY, __("Call a hook") + std::string(": "));
		this->hookText =
			new wxTextCtrl(callPanel, wxID_ANY, "");
		...
		wxTextCtrl *codeText =
		    new wxTextCtrl(splitter, wxID_ANY, code, wxDefaultPosition, wxDefaultSize,
		...
		this->shellText = 
			new wxTextCtrl(shellPanel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize,
		                   wxTE_MULTILINE);
		...
		executeButton->Bind(wxEVT_BUTTON, &CalxFileScriptHandle::OnExecuteClick, this);
	...
	void CalxFileScriptHandle::OnCallClick(wxCommandEvent &evt) {
		...
		this->recv->sendAction(std::make_unique<CalxScriptHookCallAction>(*this->script, hook));
	}
	
	void CalxFileScriptHandle::OnExecuteClick(wxCommandEvent &evt) {
		std::string code = this->shellText->GetValue().ToStdString();
		this->recv->sendAction(std::make_unique<CalxScriptExecuteAction>(*this->script, code));
	}
	...
	CalxScriptShellHandle::CalxScriptShellHandle(wxWindow *win, wxWindowID id,
		CalxActionReceiver *recv, std::unique_ptr<CalXScript> script)
		: CalxScriptHandle::CalxScriptHandle(win, id), recv(recv), script(std::move(script)) {
		...
		executeButton->Bind(wxEVT_BUTTON, &CalxScriptShellHandle::OnExecuteClick, this);
