...
TEST_CASE("Basic coord translator coordinates <-> motor position", "[translator]") {
  const motor_coord_t CENTER_X = 10;
  const motor_coord_t CENTER_Y = 5;
  const motor_coord_t SCALE_W = 2;
  const motor_coord_t SCALE_H = 3;
  BasicCoordTranslator translator(motor_point_t(CENTER_X, CENTER_Y), motor_size_t(SCALE_W, SCALE_H));
  ...
  SECTION("Motor position to coordinates") {
    ...
    REQUIRE(coordinates.x == static_cast<double>(position.x - CENTER_X) / SCALE_W);
    REQUIRE(coordinates.y == static_cast<double>(position.y - CENTER_Y) / SCALE_H);
  }
}

TEST_CASE("Basic coord translator coordinates <-> floating point position", "[translator]") {
  const motor_coord_t CENTER_X = 10;
  const motor_coord_t CENTER_Y = 5;
  const motor_coord_t SCALE_W = 2;
  const motor_coord_t SCALE_H = 3;
...
TEST_CASE("Linear coord translator without base", "[translator]") {
  ...
  SECTION("Translation with no changes") {
    ...
    coord_point_t backwards_float_position = translator.floatGet(float_position);
...
TEST_CASE("Linear coord translator with base", "[translator]") {
  std::shared_ptr<BasicCoordTranslator> basic = std::make_shared<BasicCoordTranslator>(motor_point_t(1, 2), motor_size_t(3, 3));
  const double X = -3;
  const double Y = 6;
  const double W = 5;
  const double H = 0.2;
  LinearCoordTranslator translator(coord_point_t(X, Y), coord_scale_t(W, H), basic);
